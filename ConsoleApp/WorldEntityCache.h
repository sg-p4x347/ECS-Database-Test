...
public:
	WorldEntityCache(unsigned long componentMask) : m_componentMask(componentMask) {}
	~WorldEntityCache() {}
	unsigned long GetComponentMask() {
...
public:
	WorldEntityIterator(CacheType & wec, int offset) : m_wec(wec), m_regionIterator(wec.m_regionCaches.begin()), m_offset(offset), m_entityIterator(m_regionIterator->second.begin()) {
		UpdateIterator();
	}
...
private:
	...
	EntityIterator<CompTypes...> m_entityIterator;
private:
	void UpdateIterator() {
		if (m_entityIterator == m_regionIterator->second.end()) {
			do {
				++m_regionIterator;
			...
			if (m_regionIterator != m_wec.m_regionCaches.end())
				m_entityIterator = m_regionIterator->second.begin();
		}
	}
};
